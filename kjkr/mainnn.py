    ...
    try:
        for TARGETED_ACCOUNT_UID in TARGETED_ACCOUNT_UIDs:
            ...
            if response is not None and response_nickname is not None:
                ...
                new_symbol         = position_result.index.difference(previous_symbol.get(TARGETED_ACCOUNT_UID, pd.Index([])))
                if not is_first_runs[TARGETED_ACCOUNT_UID] and not new_symbol.empty:
                    for symbol in new_symbol:
                        send_new_position_message(symbol, position_result.loc[symbol], nickname)

                closed_symbol      = previous_symbol.get(TARGETED_ACCOUNT_UID, pd.Index([])).difference(position_result.index)
                if not is_first_runs[TARGETED_ACCOUNT_UID] and not closed_symbol.empty:
                    for symbol in closed_symbol:
                ...
                previous_symbol[TARGETED_ACCOUNT_UID] = position_result.index.copy()
